...
class AngularMarginHead(nn.Module):
    ...
    def forward(self, features: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        output = torch.where(
            one_hot == 1,
            cosine_after_positive_modulator.float(),
            cosine_after_negative_modulator.float(),
        )
...
class MVArcSoftmax(AngularMarginHead):
    ...
    def negative_cosine_similarity_modulator(
        ...
        """
        return torch.where(
            cosine_after_positive_modulator.float() - self.cosine.float() >= 0,
            self.cosine.float(),
            float(self.t) * self.cosine.float() + float(self.t) - 1.0,
        )
...
class CurricularFace(AngularMarginHead):
    ...
    def negative_cosine_similarity_modulator(
        ...
        return torch.where(
            cosine_after_positive_modulator.float() - self.cosine.float() >= 0,
            self.cosine.float(),
            self.cosine.float() ** 2 + float(self.t) * self.cosine.float(),
        )
